...
def test_weather_query_uses_tool_instead_of_freshness_fallback() -> None:
    ...
    assert payload["tool_calls"][0]["tool_name"] == "weather"
    assert payload["tool_calls"][0]["status"] == "success"
    assert any(item["source_type"] == "tool" and item["tool_name"] == "weather" for item in payload["citations"])


def test_scenic_route_and_itinerary_branches() -> None:
    scenic = client.post("/api/v1/chat", json={"message": "象鼻山今天几点关门？"}).json()
    assert scenic["intent"] == "scenic_info"
    assert scenic["tool_calls"][0]["status"] == "success"

    route = client.post("/api/v1/chat", json={"message": "从桂林北站到阳朔西街怎么走？"}).json()
    assert route["intent"] == "route"
    assert route["tool_calls"][0]["tool_name"] == "route"
    assert route["grounded"] is True

    itinerary = client.post("/api/v1/chat", json={"message": "桂林两天怎么玩，帮我安排行程"}).json()
...
def test_ood_question_still_falls_back_without_tool_hallucination() -> None:
    payload = client.post("/api/v1/chat", json={"message": "火星基地量子电梯维护周期是多少？"}).json()
...
def test_stream_meta_exposes_agent_decision_and_tool_trace() -> None:
    with client.stream("POST", "/api/v1/chat/stream", json={"message": "明天桂林天气怎么样？"}) as response:
